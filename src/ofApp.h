...
// 이를 위해 미리 각 키들의 문자열 배열을 선언해, 
// for loop로 간결하게 key의 입력을 검사할 수 있다는 장점이 있다.
...
class ofApp : public ofBaseApp{
	...
	public:
		...
		void activatedkey_draw(); // 현재 누르는 키들을 화면에 그려줌
		void print_window(); // 프로그램에 쓰이는 문자열을 화면에 출력

		void sound_load(); // 필요한 sound file들을 로딩해주는 역할

		void image_load(); // 필요한 image들을 load하는 함수

		void perfect_timing_init(int level); // perfect_timing 게임 초기화
		void perfect_timing_end(); // 게임 종료 시 관련 메모리 해제
		void perfect_timing_gameover(); // 게임오버 시 실행되는 함수, 기록 저장 및 파일에 출력 등의 역할
		void perfect_timing_gameover_draw(); // 게임오버 화면을 그려주는 함수
		void perfect_timing_draw(); // 게임의 음표들을 그려주는 함수
		void calculate_timing(int key_type, int key_num); // 음표에 맞게 얼마나 잘 눌렀는지 타이밍을 검사하는 함수
